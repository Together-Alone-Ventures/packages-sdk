...
function mapOptionalCommercialStatus(raw: unknown): string | null {
  const opt = raw as [Record<string, unknown>] | [] | undefined;
  if (!opt || opt.length === 0) {
    return null;
  }
  return variantKey(opt[0]);
}
...
function mapSecuritySurface(raw: Record<string, unknown>): SecurityStatusView {
  ...
  return {
    securityStatus: variantKey(raw.security_status as Record<string, unknown>),
    commercialStatus: mapOptionalCommercialStatus(raw.commercial_status),
    deploymentId: (raw.deployment_id as [string] | [])[0] ?? null,
    orgId: (raw.org_id as [string] | [])[0] ?? null,
    moduleHash,
...
export function mergeCommercialPreflight(
  ...
  const commercialStatus =
    security.commercialStatus ?? allowance.commercialStatus ?? security.securityStatus;
  return {
    commercialStatus,
